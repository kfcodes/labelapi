...
async def print_pallet_label(pallet_id: int, request: Request):
    ...
    response = await main_pallet_label_function(printers['pallet_label_printer'], pallet_id)
    return response
...
async def print_large_combined_label_function(data: Request):
    ...
    response = await print_combined_pallet_label(json_data, printers['pallet_label_printer'])
    return response

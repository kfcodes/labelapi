...
async def upload_box_label_structures_to_printers(printers, load_box_label_variables):
    try:
        ...
        compiled_template_zpl = apply_zpl_placeholders(template_zpl, load_box_label_variables, print_output=True)
...
def load_label_template_from_env(print_output: bool = False) -> str:
    ...
    if not template_str:
        raise ValueError("BOXLABELSTRUCTURES environment variable is not set or is empty.")
...
def apply_zpl_placeholders(template_str: str, field_map: Dict[str, Union[str, int]], print_output: bool = False) -> str:
    """
